...
class InMemoryShopRepository(AbstractShopRepository):
    ...
    async def get_by_seller_id(self, seller_id: str) -> list[Shop]:
        return [s for s in self._store.values() if s.seller_id == seller_id]
...
class InMemoryRBACRepository(AbstractRBACRepository):
    def __init__(self):
        ...
        self.user_roles = {}
        self.sections = {"/client/dashboard": Section(id=1, name="Dashboard", route_name="/client/dashboard")}
    ...
    async def get_accessible_sections_for_user(self, user_id: str) -> list[Section]:
        user_role_ids = [ur.role_id for ur in self.user_roles.get(user_id, [])]
        access_sec_ids = [sec_id for (r_id, sec_id) in self.grants if r_id in user_role_ids]
        return [s for s in self.sections.values() if s.id in access_sec_ids]
    async def check_user_access_to_component(self, user_id: str, component_id: str) -> bool:
        return True
...
class InMemoryFavoriteShopRepository(AbstractFavoriteShopRepository):
    ...
    async def remove_favorite(self, client_id: str, shop_id: int) -> bool:
        to_del = [k for k, v in self._store.items() if v.client_id == client_id and v.shop_id == shop_id]
        for k in to_del:
            del self._store[k]
        return len(to_del) > 0
...
def make_order_use_case(shop_repo=None):
    ...
    return ManageOrderUseCase(order_repository=InMemoryOrderRepository(), shop_repository=shop_repo)
...
async def test_register_seller_profile():
    ...
    result = await use_case.register_seller(
        SellerRegisterDTO(id="seller_1", nic_front="front.jpg")
    )
    assert result.id == "seller_1"
...
async def test_shop_crud_operations():
    ...
    # Create
    shop = await uc.create_shop(ShopCreateDTO(seller_id="seller_1", shop_name="Test Shop", city="Kandy"))
    ...
    # Update
    updated = await uc.update_shop(ShopUpdateDTO(shop_id=1, shop_name="Updated Shop", city="Kandy"))
    assert updated.shop_name == "Updated Shop"
    
    # List
    shops = await uc.get_shops_by_seller("seller_1")
    assert len(shops) == 1
...
async def test_order_workflow_and_rating_recalc():
    shop_repo = InMemoryShopRepository()
    await shop_repo.create(Shop(seller_id="seller_1", shop_name="Tailor", city="Colombo", shop_id=1))
    
    uc = make_order_use_case(shop_repo=shop_repo)
    
    # 1. Create request
    req = await uc.create_clothing_request(
        ClothingRequestCreateDTO(client_id="client_1", clothing_category="Suit", target_budget=100.0),
        target_shop_ids=[1]
    )
    ...
    # 2. Bid
    bid = await uc.submit_bid(BidCreateDTO(shop_request_id=shop_req_id, bid_amount=120.0))
    
    # 3. Accept Bid -> Order
    order = await uc.accept_bid_and_create_order(OrderCreateDTO(shop_request_id=shop_req_id, accepted_price=120.0))
    assert order.order_status == "in_progress"
    
    # 4. Rate (should recalc average rating)
    rating = await uc.submit_rating(RatingCreateDTO(order_id=order.order_id, client_id="client_1", shop_id=1, rating=4))
...
async def test_support_operations():
    ...
    # Notifications
    n = await uc.create_notification(NotificationCreateDTO(user_id="user_1", title="Hello", message="Test"))
